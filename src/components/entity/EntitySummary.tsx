...
function StatRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex justify-between items-start gap-2 py-2 border-b" style={{ borderColor: "var(--border)" }}>
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="space-y-1 pt-2">
        ...
        <div className="text-base font-bold text-foreground leading-tight">{customer.name}</div>
      ...
      >
        ...
        <div className="text-xs text-muted-foreground mt-1">Cross-Rail Score</div>
      </div>

      {/* Primary drivers */}
      {result.primaryDrivers.length > 0 && (
        <div>
          <div className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Top Drivers</div>
      ...
      {/* Key stats */}
      <div>
        <div className="text-xs font-semibold text-muted-foreground mb-1 uppercase tracking-wider">Key Stats</div>
        <StatRow
          label="Exchange txs"
